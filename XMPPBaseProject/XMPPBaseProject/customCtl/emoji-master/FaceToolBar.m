...
-(id)initWithFrame:(CGRect)frame superView:(UIView *)superView{
    ...
    if (self) {
        ...
        [voiceButton setBackgroundImage:[UIImage imageNamed:@"icon_mac"] forState:UIControlStateNormal];
        [voiceButton addTarget:self action:@selector(didClickedAudioButton:) forControlEvents:UIControlEventTouchUpInside];
        ...
        recordButton.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin|UIViewAutoresizingFlexibleTopMargin;
        UIImage *recordImage=[UIImage imageNamed:@"chat_bottombar_voice_normal"];
        UIImage *newImage=[recordImage stretchableImageWithLeftCapWidth:10 topCapHeight:16];
        [recordButton setBackgroundImage:newImage forState:UIControlStateNormal];
        [recordButton setTitle:@"按住 说话" forState:UIControlStateNormal];
        recordButton.titleLabel.font=[UIFont boldSystemFontOfSize:16];
        [recordButton addTarget:self action:@selector(actionToStartRecord:) forControlEvents:UIControlEventTouchDown];
        [recordButton addTarget:self action:@selector(actionToEndRecord:) forControlEvents:UIControlEventTouchUpInside];
        [recordButton addTarget:self action:@selector(actionToCancelRecording:) forControlEvents:UIControlEventTouchDragOutside];
        [recordButton addTarget:self action:@selector(willCancel) forControlEvents:UIControlEventTouchDragExit];
        recordButton.frame = CGRectMake(40,toolBar.bounds.size.height-38.0f,205,buttonWh);
        ...
        moreButton.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin|UIViewAutoresizingFlexibleTopMargin;
        moreButton.enabled=YES;
        [moreButton setBackgroundImage:[UIImage imageNamed:@"chat_input_plus_normal"] forState:UIControlStateNormal];
        [moreButton setBackgroundImage:[UIImage imageNamed:@"chat_input_plus_sending"] forState:UIControlStateSelected  ];
        [moreButton addTarget:self action:@selector(chatInputMore) forControlEvents:UIControlEventTouchUpInside];
        moreButton.frame = CGRectMake(toolBar.bounds.size.width - 7.0f-30,toolBar.bounds.size.height-30-8.0f,buttonWh,buttonWh);
