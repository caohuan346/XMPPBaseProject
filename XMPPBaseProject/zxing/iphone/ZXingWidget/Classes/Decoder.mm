...
- (void)willDecodeImage {
  if ([_delegate respondsToSelector:@selector(decoder:willDecodeImage:usingSubset:)]) {
    [_delegate decoder:self willDecodeImage:self.image usingSubset:self.subsetImage];
  }
}

- (void)didDecodeImage:(TwoDDecoderResult *)result {
  if (_delegate && [_delegate respondsToSelector:@selector(decoder:didDecodeImage:usingSubset:withResult:)]) {
    [_delegate decoder:self didDecodeImage:self.image usingSubset:self.subsetImage withResult:result];
  }
  [result release];
}
...
//    NSLog(@"%s", __FUNCTION__);
    if (!self) return;
    if (_delegate && [_delegate respondsToSelector:@selector(decoder:failedToDecodeImage:usingSubset:reason:)] ) {
        [_delegate decoder:self failedToDecodeImage:self.image usingSubset:self.subsetImage reason:reason];
    }
}
